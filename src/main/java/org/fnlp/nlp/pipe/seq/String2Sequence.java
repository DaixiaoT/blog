...
public class String2Sequence extends Pipe implements Serializable {
    ...
    public static String[][] genSequence(String sent){
        ...
        Chars.CharType[] tags = Chars.getType(sent);
        int len = sent.length();
        ArrayList<String> words = new ArrayList<String>();
        ArrayList<String> types = new ArrayList<String>();
        int begin =0;
        for(int j=0; j<len; j++) {
            if(j<len-1 && tags[j]== Chars.CharType.L && tags[j+1]== Chars.CharType.L){//当前是连续英文
                continue;
            }else if(j<len-1 &&tags[j]== Chars.CharType.D && tags[j+1]== Chars.CharType.D){//当前是连续数字
                continue;
            }
            Chars.StringType st = Chars.char2StringType(tags[j]);
